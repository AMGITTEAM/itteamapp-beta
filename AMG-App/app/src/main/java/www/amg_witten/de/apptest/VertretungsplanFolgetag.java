...
public class VertretungsplanFolgetag extends AppCompatActivity
    ...
    public static void action(final Activity thise){
        ...
        final File lFile = new File(Environment.getExternalStorageDirectory() + "/Download/VertretungsplanFolgetag.htm");
        try {
            Looper.prepare();
            ...
            urlEndings.add(next);
            while(!exit) {
                URL mainUrl = new URL("https://www.amg-witten.de/fileadmin/VertretungsplanSUS/Folgetag/subst_"+next);
            ...
            for (int i=0;i<urlEndings.size();i++) {
                URL mainUrl = new URL("https://www.amg-witten.de/fileadmin/VertretungsplanSUS/Folgetag/subst_"+urlEndings.get(i));
                BufferedReader in = new BufferedReader(new InputStreamReader(mainUrl.openStream()));
                StringBuilder full = new StringBuilder();
                String str;
                while ((str = in.readLine()) != null) {
